...
- **特徴**:
  ...
  - リアルタイムストリーミング表示
- **アクセス方法**: `?user=file` パラメータで隠しモードを有効化
- **実装**: `public/modules/content-upload-reviewer.js`, `api/content-upload-reviewer.js`, `public/utils/file-parser.js`

#### 5. レジュメビルダー（Resume Builder）

AIとの対話形式でプロジェクト経験を作成する機能です。転職ドラフトの「プロジェクト経験作成サポート」と同様の機能を提供します。

- **対象**: エンジニアの職務経歴書・レジュメ作成
- **機能**:
  - AIが6つの質問を順番に投げかける対話形式
  - プロジェクト概要、体制、背景・課題、取り組み内容、成果、使用技術を段階的にヒアリング
  - 回答内容をもとに構造化されたプロジェクト経験の文章を自動生成
  - チャット形式のリアルタイム対話
  - 生成されたプロジェクト経験のコピー・ダウンロード機能（Markdown形式）
- **特徴**:
  - ストリーミング形式でAIの質問・提案をリアルタイム表示
  - 会話履歴をクライアント側で管理（ステートレス設計）
  - 約10分程度で魅力的なプロジェクト経験の文章が完成
- **アクセス方法**: `?user=resume` パラメータで隠しモードを有効化
...
1. **アクセス**: `https://json-ld-view.vercel.app/?user=file` でアクセス
2. **アップロードボタンをクリック**: ヘッダーの「コンテンツをアップロード」ボタンをクリック
3. **レビュー種類を選択**: ブログ、求人票、スキルシート、マッチング、汎用から選択
4. **コンテンツ入力**:
   - テキスト入力: テキストエリアに直接貼り付け
   - ファイルアップロード: ドラッグ&ドロップまたはファイル選択
5. **レビュー開始**: 「レビュー開始」ボタンをクリック
6. **結果確認**: Diff形式で元の内容と校閲済みの内容を比較
7. **エクスポート**: 必要に応じて校閲済みテキストをコピーまたはダウンロード
...
1. **アクセス**: `https://json-ld-view.vercel.app/?user=resume` でアクセス
2. **ボタンをクリック**: ヘッダーの「レジュメビルダー」ボタンをクリック
3. **対話開始**: AIが最初の質問を投げかけます
4. **質問に回答**: 6つの質問（プロジェクト概要、体制、背景・課題、取り組み、成果、使用技術）に順番に回答
5. **文章生成**: すべての質問に回答すると、AIがプロジェクト経験の文章を自動生成
6. **確認・編集**: 生成された文章を確認し、必要に応じて対話を続けて改善
7. **エクスポート**: 完成したプロジェクト経験をコピーまたはダウンロード（Markdown形式）

**Tips**:
- 質問に対する回答が短い場合、AIがより詳しい情報を求めることがあります
- 数値的な成果（パフォーマンス改善、売上向上など）があれば積極的に記載してください
- 技術的な工夫や課題解決のプロセスを具体的に説明すると、より魅力的な文章になります
...
個人情報を扱うベータ機能は、URLパラメータによる隠しモードでアクセスします。

### アクセス方法

各機能は独立したURLパラメータでアクセスします：

```bash
# Content Upload Reviewer（ファイル・テキストレビュー）
https://json-ld-view.vercel.app/?user=file

# My Skill Sheet（スキルシート管理）
https://json-ld-view.vercel.app/?user=skill

# Resume Builder（レジュメビルダー）
https://json-ld-view.vercel.app/?user=resume
```

### 実装詳細

- **トリガー**: URLクエリパラメータ `?user=xxx`
- **表示制御**: `public/app.js` で各機能ボタンの表示/非表示を制御
- **デフォルト動作**: パラメータがない場合、すべての隠しモード機能ボタンは `display: none` で完全非表示
- **セキュリティ**: 各機能は独立した鍵で管理され、推測されにくい

### 特徴

- **覚えやすい**: `user=file`, `user=skill`, `user=resume` とシンプルな命名
- **独立性**: 各機能は独立したパラメータで制御され、相互に影響しない
- **推測困難**: 一般的な `admin=true` などではなく、機能固有のパラメータを使用

### テスト

`test-admin-mode.html` で各隠しモードの動作を確認できます。
