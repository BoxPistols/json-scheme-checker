...
各アドバイザーには2層のレート制限が実装されています。

#### フロントエンド側レート制限（localStorage管理）

各アドバイザーが独立して50回/24時間の制限を持ちます：
...
各アドバイザーは**独立したカウント**です。複数のアドバイザーを使用する場合、理論上は各々が50回まで使用できます。

#### バックエンド側レート制限（IP単位）

セキュリティ対策として、サーバー側でも IP アドレス単位での制限を実装：

| 制限対象             | 制限値         | 方式         | APIキーでのスキップ |
| -------------------- | -------------- | ------------ | ------------------- |
| JobPosting API       | 50回/24時間/IP | メモリベース | userApiKey使用時    |
| ブログレビュアー API | 50回/24時間/IP | メモリベース | userApiKey使用時    |
| Webアドバイザー API  | 50回/24時間/IP | メモリベース | userApiKey使用時    |
...
- フロントエンド側制限：**無制限**（制限なし）
- バックエンド側制限：**スキップ**（IP制限なし）

**実際の制限**:

フロント側とバック側の両方をチェックするため、より厳しい方の制限が実質的な制限となります：

- 個人利用: **50回/24時間**（バック側の IP 単位制限に従う）
- 複数人が同一 IP からアクセス（企業 LAN など）: **50回/24時間/IP**（全体で共有）

**注意**:

- Vercel環境でサーバー再起動時にメモリがリセットされるため、メモリベースの制限がリセットされます
- 本番で永続的なレート制限が必要な場合は Vercel KV または Upstash Redis の導入を推奨
