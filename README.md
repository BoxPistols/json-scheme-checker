...
**重要**: 現在実装されているサーバー側のレート制限（IPベース）は、Vercelサーバーレス環境では完全には機能しません。

#### 問題点

- サーバーレス関数は各リクエストで異なるインスタンスで実行される可能性がある
- メモリベースのレート制限（Map）はインスタンス間で共有されない
- 悪意のあるユーザーによるAPI濫用のリスクがある

#### 現在の防御策

1. **クライアント側のレート制限** (localStorage)
   - 通常モード: 10回/24時間
   - 関係者モード: 30回/24時間
   - ただし、ブラウザ側で簡単にバイパス可能
