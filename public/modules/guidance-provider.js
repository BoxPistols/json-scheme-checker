  ...
  if (!schemas || schemas.length === 0) {
    ...
    guidance.recommendations = [
      ...
      },
      {
        ...
        description: 'ブログ記事ならArticle、商品ページならProduct、など最適なタイプを使用してください',
    ...
    if (schemaScore <= 5) {
      ...
      guidance.details = [
        ...
        hasMainProperties ? '主要プロパティが部分的に設定されています' : '主要プロパティが不足しています'
      ];
      guidance.seoImpact = '低～中 - リッチスニペット表示の可能性が低くなります';
      guidance.recommendations = [
        {
          priority: '高',
          title: '必須プロパティの追加',
          description: 'name、description、image、urlなど主要プロパティをすべて含める',
          example: 'これらのプロパティがあるとリッチスニペット表示の確率が上がります'
        },
        {
          priority: '中',
          title: '@typeの明示',
          description: 'すべてのスキーマに@typeを明示的に指定',
          example: '例: "@type": "Article" または "@type": ["Article", "NewsArticle"]'
        }
      ];
      ...
      guidance.recommendations = [
        {
          ...
          description: '各データ定義に、「データの種類」（@type）を明記してください。これでGoogleが内容を正確に判断できます',
          example: '例えば、記事であることを明記すれば、Google検索で記事として扱われやすくなります'
        },
        {
          priority: '中',
          title: '詳細な情報を追加する',
          description: '著者名、公開日時、キーワードなど、ページの詳細情報を追加してください。これらが検索結果に表示される可能性があります',
      ...
      guidance.details = [
        ...
        'リッチスニペット表示の可能性が高い状態です'
      ];
      guidance.seoImpact = '高 - リッチスニペット表示の確率が高くなります';
      guidance.recommendations = [
        {
          priority: '低',
          title: '詳細プロパティの追加',
      ...
      guidance.seoImpact = '高 - リッチスニペット表示の確率が非常に高い状態です';
      guidance.recommendations = [
        {
          priority: '低',
          title: '維持と定期確認',
...
export function getMetaScoreGuidance(metaScore, meta, issues) {
  ...
  if (!meta.robots) {
    guidance.recommendations.push({
      priority: '中',
      title: 'Robotsメタタグの設定',
      description: 'Googleに対して、このページを検索結果に表示するか、ページ内のリンクをたどるかを指示します。通常のページでは「表示して、リンクもたどる」という設定（index,follow）を推奨します。ステージング環境など、検索結果に表示したくないページでは別の設定が必要です',
    ...
    guidance.recommendations = [
      {
        priority: '中',
        title: 'SNS共有情報の追加',
        description: 'Facebook、LinkedInなどでのシェア時に、ページのタイトル・説明・画像が正しく表示されるようにタグを設定します',
    ...
    guidance.recommendations = [
      {
        ...
        example: '不足: ' + ['title', 'description', 'image', 'url', 'type'].filter(k => !og[k]).join(', ')
      }
    ];
  } else {
