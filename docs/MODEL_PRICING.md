...
| gpt-4.1-nano |          0.00000008 |          0.00000035 | レガシー互換       |
...
- **コスト重視**: gpt-5-nano（0.05/1M 入力、0.40/1M 出力と最安）
- **レイテンシ重視**: gpt-5-nano（超低レイテンシ、temperature非対応）
- **品質重視**: gpt-4o または gpt-5（高品質、コスト高）
- **バランス重視**: gpt-4.1-mini または gpt-5-mini

#### gpt-5-nano と gpt-4.1-nano の比較（本サービス観点）

- **速度/レイテンシ**: 5-nano が最速。4.1-nano はわずかに遅い
- **コスト**: 5-nano の方が安価（参考: 入力 $0.05/1M vs 4.1-nano $0.08/1M）
- **パラメータ制御**: 5-nano は temperature/top_p 非対応、4.1-nano は MyAPI 時に調整可
- **出力傾向**: 5-nano は簡潔・要点重視。4.1-nano は旧4.1系プロンプト互換・口調調整が効きやすい
- **無料枠カウント**: 両モデルを合算で 50回/24h（localStorage 管理）

推奨: まずは 5-nano（既定）。旧4.1系互換・口調コントロールが必要なら 4.1-nano へ切替。

### gpt-5-nano のアップデートポイント

- **料金**: 入力 $0.05/1M トークン、出力 $0.40/1M トークン。従来のnano系よりも安価
- **性能**: 超低レイテンシでリアルタイム応答に最適化
- **コンテキストウィンドウ**: 40万トークン（最新アナウンス値）
- **制限事項**: temperature / top_p / frequency_penalty / presence_penalty は利用不可
- **推奨用途**: コストと速度を両立したいリアルタイムアプリケーション

### MyAPIの利用が必須なモデル

既定のサーバーAPIキーでは、以下のモデルを利用できます：

- **gpt-5-nano**（既定）
- **gpt-4.1-nano**（旧nano系との互換性を重視）
